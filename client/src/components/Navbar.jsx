...
export default function Navbar() {
  ...
  return (
    <nav className="bg-gradient-to-r from-orange-200 to-yellow-100 shadow-md p-4 flex justify-between items-center rounded-b-xl border-b-4 border-orange-300">
      <Link
        to="/"
        className="flex items-center text-2xl font-bold text-orange-600"
      >
        <PawPrint className="mr-2 text-orange-500" />
        Pawfect Home
      </Link>
      ...
      <div className="flex gap-4 text-md">
        {user ? (
          <>
            ...
            <button
              onClick={logout}
              className="px-4 py-2 bg-orange-400 text-white rounded-full hover:bg-orange-500 transition-all"
            >
              Logout
            </button>
          </>
